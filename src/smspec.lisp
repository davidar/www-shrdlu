...
(DEFUN SMVG NIL 
       ...
       (PROG (TSS TENSE) 
	     ...
	     (COND ((MEMBER TENSE
			    '((PRESENT) (IMPER) (INFINITIVE)) :TEST #'EQUAL)
		    T)
								       ;IS EMBEDDED.
		   ((EQUAL TENSE '(MODAL))
...
(DEFUN SMIT (PRONOUN) 
	...
	ON   (OR SM  ; IF WE HAVEN'T YET FOUND A
		 ...
		 (MAPL 
		  #'(LAMBDA (BACKNODE) (SMIT2 BACKNODE 0.))
								       ;REFERENT MAP DOWN THE
		  BACKREF2))
								       ;BACKREF2 (NG'S IN LAST
	DONE (SETF (GET PRONOUN 'BIND) CANDIDATES)
								       ;SENTENCE) LIST
...
(DEFUN SMNG1 NIL 
       ...
       (PROG (WORD-BEING-INTERPRETED DETERS) 
             (DECLARE (SPECIAL WORD-BEING-INTERPRETED DETERS))
	     (SETQ DETERS
		   (LIST (COND ((CQ NUMD)
				((LAMBDA (NUM) 
				         (DECLARE (SPECIAL NUM))
	...
	TOOMANY      ;SENTENCES WE'RE LOOKING AT
	     ...
	     (SETQ WHO
		   (COND ((EQ WHO NIL) 'HE)
			 ...
			 ((EQ WHO 'HE)
								       ;LIST ALREADY FOUND
			  (LIST (- LASTSENTNO 1) (+ 1 LASTSENTNO)))
			 ((OR (NOT MUNG) (EQ (CAR WHO) 1.))
			  (SETQ WHO 'HE)
			  (GO TOOFEW))
			 ((CONS (- (CAR WHO) 1) (CDR WHO)))))
	     (SETQ MUNG NIL)
	     (GO UP))) 
...
(DEFUN SMCL1 NIL 
       (PROG (SMSUB SMOB1 SMOB2 SMOBL SMCOMP RELLIST X) 
	     (DECLARE (SPECIAL SMSUB SMOB1 SMOB2 SMOBL SMCOMP RELLIST))
...
(DEFUN SMCL-MODIFIERS (WORD-BEING-INTERPRETED) 
       ...
       (COND ((NULL (GET (CAR WORD-BEING-INTERPRETED) 'FEATURES)))
...
;;;		       ;NEGATIVE AND THEN ARRANGING THAT THE FEAURE
	     ...
	     ((ISQ WORD-BEING-INTERPRETED PREP))

	     ((ISQ WORD-BEING-INTERPRETED ADJG))
...
(DEFUN SMBINDER (START-EV END-EV) 
       ...
       (SETF (GET TSS 'START=) START-EV)
       (SETF (GET TSS 'END=) END-EV)) 
